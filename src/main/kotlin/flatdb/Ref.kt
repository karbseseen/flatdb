...
@JvmInline value class Ref<S: FlatStruct>(val offset: Int) {
	...
	class Range<S : FlatStruct>(val begin: Ref<S>, val end: Ref<S>, val itemSize: Int) : Sequence<Ref<S>> {
		val size get() = (end.offset - begin.offset) / itemSize
		operator fun get(index: Int) = begin + index * itemSize
